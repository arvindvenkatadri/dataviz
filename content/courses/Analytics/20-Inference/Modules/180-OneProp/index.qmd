...
Let us take a sample from a population, chosing one *Qual* variable to sample, one which has two `levels`. We perform `counts` per `level` and take the proportion $\hat{p}$. 

The Central Limit Theorem (CLT) also works for proportions, with some differences:

 - Sample proportions $\hat{p_i}$ are also centred around population proportions
 - **Success-failure condition**: If 
  $$
  \Large{\hat{p} *n >= 10}
  $${#eq-success-falure-1}
  ...
  \Large{(1-\hat{p})*n >= 10}
  $${#eq-success-falure-2}
  are both satisfied, then the we can assume that the *sampling distribution* of the proportion is normal. And so:
  
 - The Standard Error for a sample proportion is given by 
    $$
    \Large{SE = \sqrt\frac{\hat{p}(1-\hat{p})}{n}}
    $${#eq-standard-error}
...
Also, it may be easier to calculate the proportion if we create a new variable that specifies whether the individual has [***texted every day***]{style="background-color:yellow;"} while driving over the past 30 days or not. We will call this variable `text_ind`.
...
no_helmet_text %>% drop_na() %>% 
  dplyr::summarize(prop = prop(text_ind, success = "yes"), n = n())
...
```{r}
#| ref-label: plot-no-helmet-text
#| echo: false
#| label: fig-no-helmet-text
#| fig-cap: "High-Schoolers who texted every day while driving with no helmet on!!"
```
...
::: callout-important
With proportions, we usually look for a "no difference" situation, i.e. [a ratio of unity]{style="background-color:yellow;"}!! So our NULL hypothesis would be a ratio of 1:1 for texters and no-texters, so a proportion of $0.5$!!
:::
...
- [The Standard Error (and therefore the CI) for the inference of a *proportion* is **related to the actual population proportion**]{style="background-color:yellow;"}, which is very different behaviour from that with means, where SE was just a number that depended on the sample size
