...
date: 10/Nov/2022
author: Arvind V. 
subtitle: "A pig don't get fatter, the more you weigh it!"
abstract: "Means from repeated measurements"
...
$Confidence Interval = [0.4629, 2.0432]$, which includes $1$, that we cannot reject the NULL Hypothesis that the variances are not significantly different. ( Remember: this test looks at the **ratio** of the two variances, and the NULL Hypothesis is that this ratio is $1$ ).
...
```{r}
#| label: grocery-permutation-test
#| eval: false
#| 
...
:::{.column}
```{r}
#| ref.label: grocery-permutation-test
#| echo: false
...
```{r}
#| label: grocery-permutation-plot
#| code-fold: true
#| layout-ncol: 2
#| 
ggplot2::theme_set(new = theme_custom())
...
gf_ecdf(data = null_dist_price, ~ mean, linewidth = 1) %>%
  gf_vline(xintercept = obs_diff_price, 
           colour = "red",
           linewidth = 1, title = "Null Distribution by Permutation", 
           subtitle = "Cumulative Density") %>% 
  gf_labs(x = "Difference in Means") %>% 
...
```{r}
#| label: grocery-permutation-test-less
#| layout-ncol: 2
#| code-fold: true
