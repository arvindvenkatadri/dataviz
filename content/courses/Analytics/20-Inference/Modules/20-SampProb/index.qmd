...
We see in the @fig-Sampling-Mean-Distribution-1 above that

-   the *distribution of sample-means* is centered around the
    `pop_mean`. ( Mean of the sample means = `pop_mean`!! `r emoji::emoji("scream")`)
-   That the "spread" aka `sd` of the *distribution of sample means* is within than population itself. [Well within](https://www.alice-in-wonderland.net/resources/chapters-script/alices-adventures-in-wonderland/chapter-7/) ! Curiouser and curiouser! 
...
#|   - 95% CI from Multiple Samples
#| code-fold: true
#| 
set.seed(42)
mosaic::xqnorm(p = c((1-0.68)/2, 1 - (1-0.68)/2),return = c("plot"), verbose = F)
mosaic::xqnorm(p = c((1-0.95)/2, 1 - (1-0.95)/2), return = c("plot"), verbose = F)
