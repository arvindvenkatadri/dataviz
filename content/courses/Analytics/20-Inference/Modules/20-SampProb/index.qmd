...
We will now onwards **treat** this single-variable dataset as our **Population**. So this is our *population*, with appropriate *population parameters* such as `pop_mean`, and `pop_sd`.
:::
...
`pop_mean`. Is this ***always*** true, i.e. for ***any sample***? 

Let us check: we will create 500 repeated samples, each of size 50. And calculate their mean as the *sample statistic*, giving us a data frame containing 500 `sample means`. We will then see if ***all these 500 means*** lie close to the `pop_mean`:
...
The `sample_mean`s (purple dots @fig-sample-means-sds-1) are all generally in the vicinity of the `pop_mean` (vertical black line). And since they are themselves random [because the samples are random]{.black .bg-gold}, they will have their own distribution, with a`mean` and `sd` too ,over all the 500 sample-means. 

And the `sample_sd`s are also random (see @fig-sample-means-sds-2) and [have their own distribution](https://www.researchgate.net/post/Standard_Deviation_Distribution/59032925ed99e1ba910f11ff/citation/download), around the `pop_sd`.[^5]. This distribution of the sample-sds will also have its own `mean` and `sd` too !!
...
So now let's plot the **distribution** of these 500 sample-means themselves, called [**a distribution of sample-means**]{.bg-yellow .black}. We will also plot the position of the population mean `pop_mean` parameter, the mean of the `Height` variable.
...
-   Why are we interested? *Any single sample* needs to give us a `pop_mean` estimate. And it had better be good, with small standard deviation. 
- Does the sample ***size*** matter? Does the sample mean distribution change with sample size?
...
p1 <- gf_qq( ~ result, data = samples_08_1000,
             title = "N = 8", 
             color = "dodgerblue") %>%
  gf_qqline()

p2 <- gf_qq( ~ result, data = samples_16_1000,
            title = "N = 16", 
            color = "sienna") %>%
  gf_qqline()

p3 <- gf_qq( ~ result, data = samples_32_1000,
            title = "N = 32", 
            color = "palegreen") %>%
  gf_qqline()

p4 <- gf_qq( ~ result, data = samples_64_1000,
            title = "N = 64", 
            color = "violetred") %>%
  gf_qqline()
...
Let us calculate the means of the **sample-mean-distributions**:
...
mean(~ result, data  = samples_08_1000) # Mean of means!!!;-0
mean(~ result, data  = samples_16_1000)
mean(~ result, data  = samples_32_1000)
mean(~ result, data  = samples_64_1000)
pop_mean
...
For that, we need to look at the *standard deviations* of the sampling distributions:
...
Consider once again the *standard deviations* in each of the **sample-mean-distributions** that we have generated. As we saw these decrease with sample size.
...
that sample-means are *normally distributed* around the `pop_mean`, we can say that:

- $68\%$ of *all possible sample-mean* lie within
$\pm 1* SE$ of the *population mean*; 
- $95 \%$ of *all possible sample-mean* lie within $\pm 2 * SE$ of the *population mean*. 
- These two multiplier-constants $\pm 1$ and $\pm 2$ are the `z-scores` from the *z-distribution* we saw earlier:
...
#| 
set.seed(42)
mosaic::xqnorm(p = c((1-0.68)/2, 1 - (1-0.68)/2),return = c("plot"), verbose = F) %>% gf_labs(x = "Z-score")
mosaic::xqnorm(p = c((1-0.95)/2, 1 - (1-0.95)/2), return = c("plot"), verbose = F) %>% gf_labs(x = "Z-score")
TeachingDemos::ci.examp(mean.sim = 100, sd = 2,n = 50, reps = 50)

```
In @fig-confidence-intervals-examples-3, we see that $95\%$ confidence intervals straddle the population mean shown in black, almost always. One sample in 50 has missed it and is shown in pink. For a particular sample with $\bar{x} = 100$, the vertical red lines show the confidence interval derived from *that* sample. 
...
gf_annotate("curve", # CI
            x = xq[2], y = 0.09, 
            xend = xq[2] - 2, yend = 0.016,
            curvature = - 0.25, color = "purple", 
            arrow = arrow(length = unit(0.25,"cm"))) %>% 

gf_annotate(geom = "label", x = 163.55, y = 0.06, 
...
- Every sample we could take sets up a Gaussian distribution for the estimate of the `pop_mean`. (Which is `sample_mean` in this case.)
- Each sample will have a different `sample_mean`, and therefore the distribution curves, being centred on `sample_mean` would move to the right or to the left. 
- Each such sample distribution sets up a Confidence Interval. 
- We expect that 95% of the samples will setup a Confidence Interval that contains the `pop_mean`.
...
- So $$ \boxed{\Large{\mu_{pop} \sim \mu_{sample} \pm 2.0 * \sigma_{sample}}}$$
