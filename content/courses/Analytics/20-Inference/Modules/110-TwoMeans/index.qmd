...
MathAnxiety %>% head(10) %>% tt() %>% format_tt(quarto = TRUE)
glimpse(MathAnxiety)
MathAnxiety_inspect <- inspect(MathAnxiety)
MathAnxiety_inspect$categorical %>%  tt() %>% format_tt(quarto = TRUE)
MathAnxiety_inspect$quantitative %>%  tt() %>% format_tt(quarto = TRUE)
...
xqnorm(p = c(0.025, 0.975),  mean = obs_diff_amas, sd = std_error_diff,
  ...
  gf_refine(scale_y_continuous(limits = c(0, 0.8)), 
  ...
  ## Observed Difference
  gf_vline(xintercept = obs_diff_amas, color = "purple") %>% 
  gf_annotate(geom = "label", x = obs_diff_amas, y = 0.8, 
            label = "Observed Mean Difference", colour = "purple") %>% 
## Null Hypothesis
  gf_vline(xintercept = mean_diff_belief, color = "red") %>% 
...
mosaic::t_test(AMAS ~ Gender, 
               data = MathAnxiety) %>% 
  broom::tidy() %>% tt() %>% format_tt(quarto = TRUE) %>% 
...
wilcox.test(AMAS ~ Gender, 
               data = MathAnxiety) %>% 
  broom::tidy() %>% 
  tt() %>% format_tt(quarto = TRUE) %>% 
  style_tt(i = 1,j = 2,background = "gold",color = "black")
...
lm(AMAS ~ Gender, 
  ...
  broom::tidy(conf.int = TRUE,
              conf.level = 0.95) %>% 
  tt() %>% format_tt(quarto = TRUE) %>% 
  style_tt(i = 1:2, j = 5,background = "gold",color = "black")
...
lm(rank(weight) ~ gender, 
   data = yrbss_select_gender) %>% 
  broom::tidy(conf.int = TRUE,
              conf.level = 0.95) %>% tt() %>% format_tt(quarto = TRUE) %>% 
  style_tt(i = 1,j = 5,background = "gold",color = "black")
