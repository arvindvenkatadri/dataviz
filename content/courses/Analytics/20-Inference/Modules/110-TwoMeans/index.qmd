...
## Use available fonts in ggplot text and label geoms too!
...
MathAnxiety %>% head(10) %>% tt()
glimpse(MathAnxiety)
MathAnxiety_inspect <- inspect(MathAnxiety)
MathAnxiety_inspect$categorical %>%  tt() 
MathAnxiety_inspect$quantitative %>%  tt() 
...
mosaic::t_test(AMAS ~ Gender, 
               data = MathAnxiety) %>% 
  broom::tidy() %>% tt() %>% 
  style_tt(i = 1,j = 5,color = "black", bold = TRUE, background = "gold")
...
wilcox.test(AMAS ~ Gender, 
  ...
  broom::tidy() %>% 
  tt() %>% 
  style_tt(i = 1,j = 2,color = "black", bold = TRUE, background = "gold")
...
lm(AMAS ~ Gender, 
  ...
  broom::tidy(conf.int = TRUE,
              conf.level = 0.95) %>% 
  tt() %>% 
  style_tt(i = 1:2, j = 5,color = "black", bold = TRUE, background = "gold")
