...
`levels`, and of course `DeathPenalty` has two:
...
```{r}
#| label: fig-vcd1
#| fig-cap: vcd Mosaic Plot of Death Penalty by Education
#| code-fold: true
#| out-width: "75%"
#| out-height: "75%"
#| fig-align: center
...
chisq_visstat <- visstat(x = GSS2002_modified, #dataframe
                         y = "Education", # y variable
                         "DeathPenalty")# x variables
...
# Already computed
# contingency_table <- vcd::structable(Education ~ DeathPenalty, data = GSS2002_modified)
...
that proportions of `DeathPenalty` (opinion) are affected by `Education`.
...
vcd::mosaic(gss_vcd_table,gp = shading_max, legend = TRUE, direction = "v",
    labeling = labeling_border(
      varnames = c("F", "F"), # Remove variable name labels
      rot_labels = c(90, 0, 0, 0), # t,r,b,l?
      just_labels = c(
...
vcd::mosaic(gss_vcd_table, type = "expected", gp = shading_max, legend = TRUE, direction = "v",
    labeling = labeling_border(
      varnames = c("F", "F"), # Remove variable name labels
      rot_labels = c(90, 0, 0, 0), # t,r,b,l?
      just_labels = c(
...
vcd::assoc(contingency_table, gp = shading_max, legend = TRUE, direction = "v",
    labeling = labeling_border(
      varnames = c("F", "F"), # Remove variable name labels
      rot_labels = c(90, 0, 0, 0), # t,r,b,l?
      just_labels = c(
...
```{r}
#| label: chisq-test-object-structure
...
#| - "Expected"
#| - "Pearson Residuals"
#| - "Squared Residual Contribution to Chi-Square Statistic"
#| code-fold: true
...
#| out-height: "75%"
#| fig-align: center
#| fig-cap: Normal to Chi-Square variable transformation
#| fig-subcap: 
#| - Normal Density
...
```{r}
#| label: fig-Difference-in-Proportion
#| fig-cap: Observed Difference in Proportion
#| layout: [[20],[80]]
#| code-fold: true
#| warning: false
#| 
...
```{r}
#| label: fig-Shuffle-the-data-once
#| code-fold: true
set.seed(1947)
data_shuffled <- data %>%
