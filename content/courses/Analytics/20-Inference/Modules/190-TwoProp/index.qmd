...
[**Contingency
Table**](../../../10-Descriptive/Modules/40-CatData/index.qmd#what-is-a-contingency-table).
...
##
gss_table <- vcd::structable(DeathPenalty ~ Education, data = gss2002) %>%
  addmargins()
gss_table 
...
The Contingency Table can be plotted using a `mosaic` plot using several packages, [as we have seen](../../../10-Descriptive/Modules/40-CatData/index.qmd). Let us do a quick recap:
...
As discussed in the Descriptive Analysis on [Proportions](../../../10-Descriptive/Modules/40-CatData/index.qmd#using-visStatistics), `visStatistics` is a recent package that allows a very wide variety of statistical charts to be created **automagically** based on the variables chosen. Let us plot a mosaic chart directly with this package: with one function `visstats()`, we obtain mosaic and bar charts, as well as a statistical analysis of the data. We will discuss this analysis shortly. 

```{r}
#| label: visstat-1

visstat(GSS2002$DeathPenalty, GSS2002$Education) %>%  str()
```
...
mosaic::xchisq.test(vcd::structable(Education ~ DeathPenalty, data = gss2002))
# Get the observed chi-square statistic
X_squared_observed <- mosaic::chisq(mosaic::xchisq.test(vcd::structable(Education ~ DeathPenalty, data = gss2002)))
X_squared_observed
...
#| eval: false
#| echo: false
vcd::structable(Education ~ DeathPenalty, data = gss2002, type = "expected") %>%
   as.matrix() %>% # Convert to matrix;
  addmargins() %>% # Add margins to the table; matrix output
  as_tibble(rownames = "DeathPenalty") %>%  # Convert to tibble; ensure row names!
  tt()
...
These numbers are also what we saw with the code above, `mosaic::xchisq.test()`, which seems to output a format that combines both tables into one. 

And so, here are the mosaic plots for the *actual* and *expected* Contingency Tables, along with the *association* plot showing the differences, as we did when plotting [Proportions](../../../10-Descriptive/Modules/40-CatData/index.qmd#coloured-tiles-actual-and-expected-contingency-tables):
...
`z-score` of that cell]{.bg-light-red .black}. Recall the [z-score](../../../10-Descriptive/Modules/22-Histograms/index.qmd#z-scores) idea: we subtract the mean and divide by the std. deviation to get the `z-score`.
