...
```{r}
#| message: true
starwars %>% dplyr::select(home_world = homeworld) #select + rename
```
...
```{r, eval = FALSE}
#| message: true
a1 <- dplyr::group_by(starwars, species, sex)
a2 <- dplyr::select(a1, height, mass)
a3 <- dplyr::summarise(a2,
...
#| message: true
name <- "color"
dplyr::select(starwars, ends_with(name))
...
#| message: true
name <- 5
dplyr::select(starwars, name, identity(name))
