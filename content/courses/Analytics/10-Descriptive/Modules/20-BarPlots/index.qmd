...
-   Base R supports graph making out of the box; (fast, very flexible, but a bit complex)(Update July 2025: There is a new(?) package called `tinyplots` that creates base R plots with syntax that is much more intuitive. (It is very lightweight and does not require any additional packages to be installed.)

-   The most well known is `ggplot` <https://ggplot2-book.org/> which
    uses Leland Wilkinson's concept of a "Grammar of Graphics"; 
    
- `ggformula` is a wrapper around `ggplot` that makes the syntax a little more concise and intuitive. This package also uses a very similar interface to the `mosaic` package, which is a system for statistical analysis and data visualization in R. The `mosaic` package is designed to make it easier to perform statistical operations and create visualizations with a consistent syntax.

- The recently introduced `tidyplots` package, which allows intuitive creation of ***publication-ready charts*** based on the famous and established `ggplot` framework.
...
- Interactive plots are available using `plotly` <https://plotly.com/r/>, and `echarts4r`<https://echarts4r.john-coene.com/>. These packages allow for dynamic and interactive visualizations that can be embedded in web applications.

Each system has its benefits and learning complexities. We will largely look at **static plots** ( i.e not interactive) created using `ggformula`. We will also state `ggplot` code too for comparison.

### {{< iconify hugeicons api >}} Command Templates or API

Here is a brief description of the *API*: the programming interface to each of `mosaic`, `ggformula`, and `ggplot`. As stated earlier, `mosaic` and `ggformula` have a very similar, and intuitive, interface. 
