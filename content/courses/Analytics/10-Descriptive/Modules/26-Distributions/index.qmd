---
title: <iconify-icon icon="clarity:bell-curve-line" width="1.2em" height="1.2em"></iconify-icon> Distributions
...
#     wrap: 72
webr:
   packages: ['readr', 'tidyr', 'dplyr','mosaic', 'skimr', 'ggformula','ggridges', 'palmerpenguins','janitor']
format: 
  html: default
  revealjs: 
    theme: default
    output-file: distributions-reveal.html
    smaller: true
    center: false
    scroll-view: true
    scrollable: true
...
```{r}
#| label: fig-nebraska-avenue
#| fig-cap: "Lincoln, Nebraska, in the Sun"
#| fig-align: center
#| code-fold: true
#| out-width: "75%"
#| warning: false
...
We will first look at at a dataset that is available as a part of the `{{palmerpenguins}}` package (and also directly available in R now), the
...
### {{< iconify la r-project >}} Read the Data
...
#| tbl-cap: "Penguins Clean Dynamic Data Table"
#| code-fold: true
#| 
...
- `body_mass_g`: Mass in grams. Grams? Grams??? Why, these penguins are like human babies!!`r emoji::emoji("heart")`
...
penguins_clean %>% gf_density( ~ body_mass_g, 
                         fill = ~ species, 
                         color = "black") %>%
...
penguins_clean %>%
  ...
  gf_labs(x = "Body Mass of Penguins", title = "Plot D: Penguin Body Mass by Species and facetted by Sex",
...
penguins_clean <- penguins_clean %>% drop_na()
...
plot*. `{{ggformula}}` supports this plot by importing/depending upon the `{{ggridges}}` package. `{{ggridges}}` provides direct support for ridge plots, and can be used as an extension to `{{ggplot2}}` and `{{ggformula}}`.
...
gf_density_ridges(drv ~ hwy, fill = ~ drv, 
                  ...
                  data = mpg) %>% 
  gf_refine(scale_fill_brewer(name = "Drive Type",
                              palette = "Spectral")) %>% 
  gf_labs(title = "Mileage vs Drive Type",
          subtitle = "Ridge Plot", 
          x = "Highway Mileage", 
          y = "Drive Type")
...
ggplot(data = mpg, mapping = aes(x = hwy, y = drv, fill = drv)) + 
  ...
  labs(title = "Mileage vs Drive Type",
          subtitle = "Ridge Plot", 
          x = "Highway Mileage", 
          y = "Drive Type")
