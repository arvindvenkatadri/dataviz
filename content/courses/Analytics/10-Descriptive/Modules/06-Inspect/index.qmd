...
Our first task is to get acquainted with our data, to check the variables, the size of the dataset, how it is formatted, to eat it, as it were. We need to **inspect** the data, to understand what it is telling us. The physical significance of each variable needs to sink in before we can do anything with it. This is especially important in ***design***, since we may be working in domains that are not within our own range of acqaintance or expertise. It is also an important step in the data analysis process.
...
We will step-by-step proceed as follows:

- Use `readr::read_cv()` to read the data
- Variables Names: `base::names()` and `dplyr::glimpse()`
...
### {{< iconify carbon structure >}} Variables in our Dataset
...
visdat::vis_miss(fastfood) +
  ggtitle(label = "Missing Data in fastfood Dataset") + 
  theme(plot.title = element_text(size = 20, hjust = 0.5))

visdat::vis_dat(fastfood, sort_type = TRUE, palette = "cb_safe") +
  labs(title = "Missing Data in fastfood Dataset", 
...
It is clear that there are quite a few missing values in a few columns: `vit_a`, `vit_c` and `calcium`. Some missing values are also present in `fiber`. 
...
### {{< iconify carbon clean >}} Clean the Variable Names
...
We will also not touch the original data, but save the modified data in a new variable called `mpg_modified`. This is a good practice, as it allows us to keep the original data intact, and also to compare the two if needed.
...
### {{< iconify streamline dictionary-language-book-solid >}} Data Dictionary and Munging, and Printing
...
::: callout-note
### Qualitative Data
- `restaurant`(chr): Name of the restaurant
- `item`(chr): Name of the dish
- `salad` (chr): Is it a salad? (Yes/No)
- `rownames`(int): Row ID
:::
...
fast_food_modified <- fast_food_modified %>%
  mutate(
    ...
    item = as.factor(item) # Name Change too!!
  ) %>%
...
fast_food_modified %>%
  head(10) %>% 
  tt(caption = "Fast Food Dataset (Clean)") %>%
  theme_html(class = "table table-hover table-striped table-condensed") %>% 

  setNames(c("Row ID", "Restaurant", "Dish", "Calories", "Calories from fat","Total Fat (g)", "Saturated Fat (g)", "Trans Fat (g)", "Cholesterol (mg)", "Sodium (mg)", "Carbohydrates (g)", "Fiber (g)", "Sugars (g)", "Protein (g)", "Vitamin A (% DV)", "Vitamin C (% DV)", "Calcium (% DV)", "Iron (% DV)"))
