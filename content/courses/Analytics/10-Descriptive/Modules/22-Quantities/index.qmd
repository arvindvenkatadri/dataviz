...
bibliography: 
  ...
  - grateful-refs.bib
webr:
  packages: ['crosstable','readr', 'tidyr', 'dplyr','mosaic', 'skimr', 'ggformula','ggplot2', 'ggridges', 'palmerpenguins', 'janitor']
format: 
  html: default
  revealjs: 
    theme: default
    output-file: quantities-reveal.html
    smaller: true
    center: false
    scroll-view: true
    scrollable: true
...
::: {}
<center>
{{< video https://vimeo.com/127511255 width="560" height="400" align="center" title="" >}}
</center>
...
::: {}
<center>
{{< video https://youtu.be/Wcqt49dXtm8 width="560" height="400" align="center" title="" >}}
</center>
...
::: {.column width="45%"}
In the figures above, the ***absolute value*** of the random variables $x_i$ is always $10$ from the (identical) means $\mu_i$. However since the $\sigma_i$ are different, the `z-score` is different in each case. In the left column, the $z_{score} = 2$; in the right column it is $z_{score} = 0.6667$.
...
::: callout-tip
Note: reading `xlsx` files into R may need the the `{{readxl}}` package. Install it, peasants!!
:::
