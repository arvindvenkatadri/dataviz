...
![[Machiavellism](https://medium.com/orikami-blog/behind-the-screens-likert-scale-visualization-368557ad72d1)](../../../../../materials/images/machiavellism.webp){#fig-machiavellism}

This figure is from by the article [Behind the Screens: Likert Scale Visualization](https://medium.com/orikami-blog/behind-the-screens-likert-scale-visualization-368557ad72d1) by [Orikami](https://orikami.com/). The article discusses how to visualize Likert scale data, which is a common type of survey data used to measure attitudes or opinions.

The figure shows a Likert scale visualization of the Machiavellism personality trait, which is a measure of how manipulative and deceitful a person is. The visualization uses a horizontal bar chart to show the distribution of responses across different levels of Machiavellism, with the bars representing the percentage of respondents who selected each age-group level.

Are there any surprises, peasants of Gantiganahalli?
...
```{r}
#| echo: false
#| label: fig-QUICK_EZ-app
#| fig-cap: "Likert Plot for the User Survey for QuickEZ"
#| fig-align: "center"
#| out-width: "75%"
#| 
ggplot2::theme_set(new = theme_custom())
...
```{r}
#| echo: false
#| label: fig-efc-survey
#| fig-cap: "Likert Plot for the User Survey for EUROFAM Healthcare Dataset"
#| fig-align: "center"
#| out-width: "75%"
#| 
...
efc %>% 
  ...
  scale_fill_brewer(name = "Responses",
    ...
    palette = "Set3", direction = -1) + theme_custom() +
    geom_vline(xintercept = 0) + 
    theme(legend.position = "bottom") 
...
```{r}
#| echo: false
#| label: fig-R-usage-survey
#| fig-cap: "Likert Plot for Do You Use R Survey"
#| fig-align: "center"
#| out-width: "75%"
#| 
set.seed(427)
...
- We can use Likert charts as a starting point for the analysis of the survey data using much the same techniques as with [Inference for Two Proportions](../../../20-Inference/Modules/190-TwoProp/index.qmd), since we *are* dealing with proportions of people who answer various questions on a Likert Scale. 
...
The package `{{sjstats}}` provides the tools to analyze survey data. 
There is also a more recent package called `{{surveydown}}`, which I need to investigate. And `{{srvyr}}` and `{{survey}}`. Sigh. 

## {{< iconify openmoji person >}} Your Turn

1.  Take some of the categorical datasets from the `{{vcd}}` and `{{vcdExtra}}` packages and recreate the plots from this module. Go to <https://vincentarelbundock.github.io/Rdatasets/articles/data.html> and type "vcd" in the `search` box. You can directly load CSV files from there, using `read_csv("url-to-csv")`.
...
3. Analyze Survey Data for Free. <https://asdfree.com/>. Forty-Nine Public Microdatasets, One easy-to-type Website.
...
1.  Stephanie Zimmer, Rebecca Powell, Isabella Velasquez (Aug 2025). *Complex Survey Data Analysis: A Tidy Introduction with {srvyr} and {survey}*. <https://tidy-survey-r.github.io/user-2025-survey-data-tutorial>. useR! 2025 tutorial held at Duke University, August 2025. 
