...
```
And we will of course clean up the variable names, not that they need cleaning this case!!
...
-   There are a few missing values in `sex`(11 missing entries) and all the Quant variables (2 missing entries each). Let us drop the rows with missing values for now.

:::

```{r}
#| label: drop-missing-penguins
penguins_modified <- penguins_modified %>% drop_na()
...
gf_density( ~ body_mass_g, data = penguins_modified) %>%
...
penguins_modified %>% gf_density( ~ body_mass_g, 
                         fill = ~ species, 
                         color = "black") %>%
...
penguins_modified <- penguins %>% drop_na()
