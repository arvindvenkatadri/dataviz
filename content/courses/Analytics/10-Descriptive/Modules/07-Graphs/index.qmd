...
- Visuals are a good starting point [to make **hypotheses**]{style="background: yellow;"} of what may be happening in the situation represented by the data
...
[equivalent to **metaphors** as in a language itself]{style="background: yellow;"}. And indeed, in R we use a philosophy called the **Grammar of Graphics**! We will use this grammar in the R graphics packages that we will encounter. Other parts of the Workflow (Transformation, Analysis and Modelling) are also following similar grammars, as we shall see. 
