...
read_csv("../../../../../materials/Data/pronouns.csv") %>% 
  filter(No == "3") %>% 
  knitr::kable(theme = "striped", caption = "Which Variables?", centering = TRUE)
...
We have already examined [Bar Charts](../20-Counts/index.qmd).
...
In this section we understand how to make Contingency Tables(CT) from each
of the three forms. We will use `base-R` and the `{{vcd}}` package for our 
purposes. Then we will see how they can be visualized. In most cases, 
we will also see how we can create the Contingency Table as a `tibble` 
or `data frame` for further analysis.
