...
library(skimr)
options(rgl.useNULL = TRUE) # reset graphics device for vcdExtra
rgl::rgl.useNULL()
...
# devtools::install_github("haleyjeppson/ggmosaic") # removed from CRAN
...
Since we will not use the ggplot system, we do not need this code chunk. 
...
read_csv("../../../../../materials/Data/pronouns.csv") %>% 
  filter(No == "3") %>% 
  tt(theme = "striped", caption = "Which Variables?") %>%  
...
<https://www.stubbornmule.net/2009/07/love-old-fashioned>](../../../../../materials/images/mosaic-chart-music.png){#fig-mosaic-inspiration}
...
From @fig-mosaic-inspiration, we see that the [1000 Songs you must hear before you die](https://www.stubbornmule.net/2009/07/love-old-fashioned) are shown in a plot of *song topic* vs *song-year*, and that the important songs vary in topic over the years. 
...
We saw with [Bar Charts](../20-Counts/index.qmd) that when we deal
...
Arthritis %>%  
  head(10) %>% 
  knitr::kable(theme = "striped", caption = "Arthritis Treatments and Effects<br> First 10 Observations",centering = TRUE)
...
1)  `ID`:` integer; a unique identifier for each case
2)  `Treatment`: a factor; Placebo or Treated
3)  `Sex`: a factor, M / F
4)  `Age`: integer; age of the patient
...
# Tibble as HTML for presentation
GSS %>%
  knitr::kable(theme = "striped", caption = "General Social Survey", 
centering = TRUE)
...
combination of the two *categorical* variables, `sex` and `party`. This is still a `data.frame`. 
...
Multiple 2-Way tables ( such as `HairEyeColor`) can be **flattened** into 
a single long table that contains all counts for all combinations of categorical variables. This can be visualized as "opening up" and laying flat the 
Rubik's cube, as with a cardboard model of it.
...
Converting these (multiple) tables into a data frame or tibble:
...
HairEyeColor %>% 
  as_tibble() %>%  # Convert
  knitr::kable(theme = "striped", 
               caption = "Hair Eye and Color<br> as a Data Frame",
               centering = TRUE)
...
We have already examined [Bar Charts](../20-Counts/index.qmd). Pie Charts are discussed [here.](../60-PartWhole/index.qmd). These are both good for *single Qual variables*. Bars are more suited
when there are many *levels* and/or when there is more than one Qual
variable, as discussed earlier.

How do we deal with **multiple Qual variables**?
...
*Table*]{style="background-color: yellow;"} , which we will use to build our plot for nested proportions. The
...
We can think of it as an ***a new intermediate tabular structure*** that will help us create a new kind of chart for our Qual data. 

::: {}
<center>
{{< video https://youtu.be/7NhNeADL8fA width="560" height="400" align="center" title="">}}
</center>
:::
...
1. **vcd**: For Case Form data, `vcd::structable()` is a good option, as it can also add margins to the table. The Table can also be converted to a tibble, preserving the rownames.

```{r}
#| label: tbl-vcd-case-form-ct
#| tbl-cap: Creating Contingency Tables from Case Form Data with `vcd`
#| layout-ncol: 2
#| code-fold: true
#| 
...
2. **Base R**: Base R also provides very compact formula-based smethods for Case Form Data:

```{r}
#| label: tbl-base-case-form-ct
#| tbl-cap: "Creating Contingency Tables from Case Form Data with `base::ftable()`"
#| layout-ncol: 2
#| code-fold: true
#| 
Arthritis %>% class()
...
But it is a **set** of (two-way) Contingency Tables. So let us try!

1. **vcd**: For Frequency Form data, `vcd::structable()` can again work nicely for us:


```{r}
#| label: tbl-vcd-freq-form-ct
#| tbl-cap: "Creating Contingency Tables from Frequency Form Data with `vcd`"
#| layout-ncol: 2
#| code-fold: true
#| 
...
as_tibble(rownames = "Admit-Dept") # Convert to tibble; ensure row names!

```

2. **Base R**: Base R also provides a very compact method for Frequency Form Data:

```{r}
#| label: fig-base-freq-form-ct
#| fig-cap: "Creating Contingency Tables from Frequency Form Data with `base::ftable()`"
#| layout-ncol: 2
#| code-fold: true
#| 
UCBAdmissions %>% class()
...
1. **vcd**: For Table Form data, `vcd::structable()` can readily be used: 

```{r}
#| label: tbl-vcd-multitable-data-ct
#| tbl-cap: "Creating Contingency Tables from Table Form Data with `vcd`"
#| layout-ncol: 2
#| code-fold: true
#| 
...
vcd::structable(data = HairEyeColor, Sex ~ Hair + Eye) %>% 
  as.matrix() %>%
  addmargins() %>% # Add margins to the table
  as_tibble(rownames = "Hair-Eye") # Convert to tibble; ensure row names!

```


2. **Base R** also offers the `ftable()` option again:

```{r}
#| label: tbl-base-multitable-data-ct
#| tbl-cap: "Creating Contingency Tables from Table Form Data with `base::ftable()`"
#| layout-ncol: 2
#| code-fold: true
#| 
HairEyeColor %>% ftable(Hair + Eye ~ Sex)
...
All right then, how does one plot a set of data that looks like this, a
...
```{r}
#| label: tbl-gss2002-data
#| tbl-cap: "GSS 2002 Data"
#| code-fold: true
#| warning: false
...
```{r}
#| label: tbl-gss2002-ct
#| tbl-cap: "GSS 2002 Contingency Table"
#| code-fold: true
#| warning: false
#| 
gss_table <- vcd::structable(Education ~ DeathPenalty, data = GSS2002)
gss_table %>% addmargins() %>% 
  as_tibble(rownames = "Education")

```



The answer is provided in [*the very shape of the CT*]{.bg-gold .black}: we plot this as
...
-   Take the [bottom row]{.bg-light-green .black} of the CT (i.e.per-column totals) and create vertical
...
```{r}
#| label: fig-mosaic-plot-steps
#| fig-cap: "Mosaic Chart Steps for GSS2002 Data"
#| fig-subcap:
#| - "Step#1: Education"
#| - "Step#2: DeathPenalty"
#| echo: false
...
vcd::structable(Education ~ 0, data = GSS2002) %>% vcd::mosaic(
  gp = shading_Friendly,
  main = "GSS2002 Education Mosaic Chart Step#1",legend = FALSE)

vcd::structable(DeathPenalty ~ Education, data = GSS2002) %>% 
vcd::mosaic(
  gp = shading_hsv, direction = "v",
  main = "GSS2002 Education Mosaic Chart Step#2", legend = FALSE)
...
bottom ROW of the @tbl-gss2002-ct. HS is clearly
the largest subgroup in `Education`.

In the second step, the columns from @fig-mosaic-plot-steps-1 are
...
COLUMN, giving us @fig-mosaic-plot-steps-2.
...
Of course, `vcd::structable()` gives us one!

```{r}
#| label:  fig-mosaic-plot-arthritis-vcd
#| fig-cap: "Arthritis Mosaic Plot"
#| layout-ncol: 2
#| code-fold: true
#| 
...
```{r}
#| label: fig-mosaic-plot-nhanes-vcd
#| fig-cap: NHANES Mosaic Plot
#| layout-nrow: 2
#| code-fold: true
#| 
library(NHANES)
...
```{r}
#| label: fig-visstat-1
#| fig-cap: "Mosaic and Bar Chart with visStatistics"
#| fig-subcap:
#|  - "Bar Chart"
#|  - "Mosaic Chart"
#| layout-ncol: 2
...
# visstat cannot take "ordinal" variables ;-O
Arthritis <- Arthritis %>% 
  mutate("Improved" = base::factor(Improved, ordered = FALSE))
Arthritis
visstat(Arthritis$Improved, Arthritis$Treatment)
...
# Plots the Bar Chart and Mosaic Chart
visstat(NHANES$Race1, NHANES$Education)

```

The `{{visStatistics}}` package is a very powerful package that can be used to create a wide variety of statistical charts, including Mosaic Charts, with very little code. This "universal" command `visstat()` computes a good many statistics and plots, all in one. NOTE: `visstat()` does need both its variables to be `integers/numbers/factors`: cannot handle `ordinal` (factor) variables.

::: callout-important

Note how one `visstat()` command gave us TWO plots! The output of `visstat()` is very interesting!
So, for the fearless: Try doing an `str()` after this command to see what (else) it has computed. There is a wealth of information in the output object, including the Contingency Table, the Chi-Squared test results, and more. Type `visstat(GSS2002$DeathPenalty, GSS2002$Education) %>% str()` in the Console or your Quarto chunk to see this.
...
gss_summary <- GSS2002 %>% 
  ...
  dplyr::mutate(edu_count = sum(count), 
...
gf_col(edu_prop ~ Education, data = gss_summary,
       ...
       position = "fill", 
       color = "black") %>% 
  gf_labs(title = "Mosaic Chart with ggformula") %>%
...
We notice that the mosaic plots have coloured some tiles blue and some red. Why was this done? Consider the set of mosaic plots below:


::: column-body-outset-right
```{r}
#| label: fig-contingency-expected-actual-diff
#| fig-cap: Contingency Table Plots
#| fig-subcap: 
...
The graph on the left @fig-contingency-expected-actual-diff-1 shows the mosaic plot of the **actual** Contingency Table. 

The graph in the middle @fig-contingency-expected-actual-diff-2 shows a similar but fictitious plot but with the cuts neatly horizontal or vertical. [This mosaic is what we would **expect**, if `Education` and the opinion on `Death Penalty` were independent!!]{style="background-color:yellow;"}

The graph on the right @fig-contingency-expected-actual-diff-3 shows the **differences** between the two plots, tile by tile.

Clearly, there are differences in area of the corresponding tiles in the two mosaics, *actual* and *expected*, as shown in the graph on the right. Some differences are positive, and some negative. In the *actual mosaic*, @fig-contingency-expected-actual-diff-1, tiles with large positive differences are coloured blue, and those with large negative differences are coloured red. These are called the ***Pearson Residuals***, and they indicate the *extent* to which the actual counts differ from the expected counts, if the two Qual variables were independent. The higher the absolute values of these differences, the greater the effect of one Qual on the other. 

We will explore these ideas (*what does "expected" mean here?*) when we get into [Inference for Two Proportions.](../../../20-Inference/Modules/190-TwoProp/index.qmd)
...
Proportions.](../../../20-Inference/Modules/190-TwoProp/index.qmd#observed-statistic-the-x2-metric).
...
ggballoonplot(df, x = "Hair", 
  ...
  scale_fill_viridis_c(option = "C") + 
  labs(title = "Balloon Plot for HairEyeColor Dataset")
```
:::
...
-   We can detect *correlation* between Quant variables using the
    [scatter plots and regression lines.](../30-Change/index.qmd)
...
-   [Your project primary research data may be ***pure Qualitative*** too, as
    with a Questionnaire / Survey instrument]{style="background-color:yellow;"}
...
3. From Kaggle, [1000 songs to hear before you die](https://www.kaggle.com/code/iampedroalz/the-guardian-s-1000-songs-to-hear-before-you-die). Can you recreate the mosaic chart shown @fig-mosaic-inspiration?
...
1.  University of Iowa. *STAT4580: Statistics and Actuarial Science.* [Visualizing Categorical Data in R](https://homepage.divms.uiowa.edu/~luke/classes/STAT4580/catone.html)\
1.  Mine Cetinkaya-Rundel and Johanna Hardin. *An Introduction to Modern Statistics*, Chapter 4. **Exploratory Analysis of Categorical Data**. <https://openintro-ims.netlify.app/explore-categorical.html>\
1.  Using the `strcplot` command from `vcd`,
    <https://cran.r-project.org/web/packages/vcd/vignettes/strucplot.pdf>\
1.  Creating Frequency Tables with `vcd`,
    <https://cran.r-project.org/web/packages/vcdExtra/vignettes/A_creating.html>\
1.  Creating mosaic plots with `vcd`,
...
1.  Chittaranjan Andrade(July 22, 2015). *Understanding Relative Risk,
...
1.  Michael Friendly, *Corrgrams: Exploratory displays for correlation
    matrices*. The American Statistician August 19, 2002 (v1.5).
    <https://www.datavis.ca/papers/corrgram.pdf>\
1.  H. Riedwyl & M. Schüpbach (1994), *Parquet diagram to plot
...
cite_packages(
  ...
  out.dir = ".",
  out.format = "html",
  pkgs = c("ggmosaic", "ggpubr", "tinytable", "vcd", "vcdExtra", "visStatistics")) %>%
