...
**Together?**: We can use Quant and Qual together, to develop the above summaries (`min`, `max`,`mean`, `median` and `sd`) for Quant variables, again **across levels, and across combinations of levels** of single or multiple Quals, along with `counts`. This will tell us if our (sample) dataset already shows *quantitative differences* between  sub-classes in the population. 
...
We can simply perform ***Counts with all the Qual variables***, to get a sense of how many `levels` each variable has, how many observations per `level`, and therefore how many ***combinations of levels*** this data could have. 

```{r}
#| label: tbl-docVisits-counts-and-tables
#| tbl-cap: "Counts of Qual Variables in `docVisits`"
#| tbl-subcap: 
#|   - "Counts of `gender`"
#|   - "Counts of `private`"
#|   - "Counts of `gender` and `private`"
#|   - "Counts of all Qual variables!!"
#| code-fold: true
#| 
...
The last output is a count of observations for each combination of levels of the 6 Qual variables. There are 24 such combinations. It should be possible using this table to understand a little about the nature of Experiment conducted by those who created this dataset.
...
```
We can set up summaries of Quant variables singly, or in pairs, or even all together. This gives us the ***relative size*** of the Quant variables in the dataset. The resultant output may not be useful in all cases, but it is easy to generate and see if anything useful emerges.
...
1. Try your hand at these datasets below. Look at the data, state the data dictionary, contemplate a few Research Questions and answer them with Summaries and Tables in Quarto!
...
Try the datasets in the package `{{CardioDataSets}}`.
```{{r}}
...
Try the datasets in the package `{{NeuroDataSets}}`.
```{{r}}
