...
editor: 
  markdown: 
    wrap: 72
webr:
  packages: ['readr', 'dplyr','forcats','mosaic', 'skimr', 'ggformula', 'janitor']
---
...
data(mpg, package = "ggplot2") # load the mpg dataset
mpg_modified <- mpg %>%
  janitor::clean_names(case = "snake") %>% # clean names
  janitor::remove_empty("cols") # remove empty columns
...
### {{< iconify carbon clean >}} Make Clean Variable Names

As a part of the process, we should make sure that the variable names are "clean": there are a few options here, such as `camelCase`, `snake_case`, `kebab-case`, or `dot.case`. We will use the `janitor` package to do this, and also to make sure that the variable names are unique. AND, we will stick with `snake_case` for the rest of this course. 

In this specific case, the variable names are already very terse and simple and this process would not change things too much. But as names in data become complex, with special characters ( %$#@! etc.), this becomes very useful.
...
Let us get acquainted with another dataset, so that we will have two reference points to compare. 
...
```{webr-r}
docVisits_modified %>% inspect()
```

```{webr-r}
docVisits_modified %>% skim()
```
...
OK, now we have two datasets. We are ready to prepare summary statistics, counts based on individual Quant variables and Qual variables, and also combinations of these. We will use the `dplyr` package from the `tidyverse` for this, and also the `mosaic` package, which has some very useful functions for summarizing data.
...
## {{< iconify vaadin group >}} {{< iconify carbon summary-kpi >}} Grouped Summaries of Quantitative Variables
...
Note that we have *Qualitative* variables as well in a typical dataset. These Qual variables help us to [**group** the entire dataset based on their combinations of **levels**]{style="background-color: yellow;"}. We can now think of *summarizing Quant variables* within each such group. 
