...
subtitle: "Outliers and other Crazy Things"
date: 24/June/2024
date-modified: today
...
#     wrap: 72
webr:
  packages: ['readr', 'tidyr', 'dplyr','forcats','mosaic', 'skimr', 'ggformula','ggridges', 'palmerpenguins', 'janitor']
format: 
  html: default
  revealjs: 
    theme: default
    output-file: groups-reveal.html
    smaller: true
    center: false
    scroll-view: true
    scrollable: true
...
![](../../../../../materials/images/1book4.jpg){height=250 fig-align="center"}
...
![Box Plot Inspiration](what-we-mean-when-we-say.jpeg){#fig-boxplot-inspiration width="560px" height="400px"}
...
::: {#fig-boxplot-skew layout-ncol=2}
...
Compare these with the corresponding distributions in @fig-distribution-skewness.



### Box Plots and Outliers

Box plots can show the presence of *outliers* in distributions, with a large number of outliers on one side, as in @fig-distributions-boxplots.
...
## This code is for generating random data shown in the figure. Do not pore over it!!
...
rand_data %>% 
  ...
  gf_boxplot(0.3 ~ value, fill = ~ type, alpha = 0.3, 
...
The data has automatically been read into the `webr` PlayGround session, so you can continue on to the next code chunk!
...
### {{< iconify file-icons influxdata >}} Inspect Data
...
It turns out that following our process of munging the data using the `{{naniar}}` package to replace common NA strings and numbers with actual `NA` makes the code run very slowly. [I need to figure out how to optimize this.]{style="background-color: yellow;"}
...
### Qualitative Data
  ...
  - [`childs`(dbl): number of children (0-8)]{style="background-color: yellow;"}
:::
...
### {{< iconify material-symbols table-chart >}} Data Table

```{r}
#| label: tbl-dt-for-data
#| tbl-cap: "GSS Wages Clean Dynamic Data Table"
#| code-fold: true
...
### {{< iconify  material-symbols lab-research >}} Experiment Description
...
### Research Questions:
  ...
  - Is `realrinc` affected [by]{style="background-color: yellow;"} `gender`? 
  - [By]{style="background-color: yellow;"} `educcat`? [By]{style="background-color: yellow;"} `maritalcat`?
  - Is `realrinc` affected [by]{style="background-color: yellow;"} `child`? 
...
See that [preposition "by"]{style="background-color: yellow;"} in the questions above? It is a good idea to use it when you are asking about the effect of a Qual variable on a Quant variable. And we know that Qual variables are the ones that define...***groups***! So we are asking about the effect of groups on a Quant variable.
...
wages_clean %>% 
  ...
  gf_labs(title = "Plot 1A: Income has a skewed distribution",
          ...
          x = "", y = "Income") #Blank out the X-axis title
```
:::
::: {.column}
```{r}
...
::: {.column}
```{r}
#| label: income-by-gender-ggformula-1
#| eval: false
#| 
...
```{r}
#| ref-label: income-by-gender-ggformula-1
#| echo: false
#| 
```
:::
...
::: {.column}
```{r}
#| label: income-by-gender-ggformula-2
#| eval: false
...
```{r}
#| ref-label: income-by-gender-ggformula-2
#| echo: false
```
:::
::::
...
::: {.column}
```{r}
#| label: income-by-gender-ggformula-3
#| eval: false
...
```{r}
#| ref-label: income-by-gender-ggformula-3
#| echo: false
...
::: {.column}
```{r}
#| label: income-by-educcat-ggformula-1
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-ggformula-1
#| echo: false
#| 
```
...
::: {.column}
```{r}
#| label: income-by-educcat-ggformula-2
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-ggformula-2
#| echo: false
...
::: {.column}
```{r}
#| label: income-by-educcat-ggformula-3
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-ggformula-3
#| echo: false
...
::: {.column}
```{r}
#| label: income-by-educcat-ggformula-4
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-ggformula-4
#| echo: false
...
```{r}
#| label: income-by-educcat-ggplot
#| layout-ncol: 2
#| layout-nrow: 2
...
:::{.column}
```{r}
#| label: income-by-educcat-childs-ggformula-1
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-childs-ggformula-1
#| echo: false
...
:::{.column}
```{r}
#| label: income-by-educcat-childs-ggformula-2
#| eval: false
...
::: {.column}
```{r}
#| ref-label: income-by-educcat-childs-ggformula-2
#| echo: false
...
### Using ggplot
```{r}
#| label: income-by-educcat-childs-ggplot
#| layout-ncol: 2
...
This is a very important idea of *Hypothesis Testing* which helps you justify your hunch. We will study this when we do Stats Tests for [differences between two means(t-tests)](../../../20-Inference/Modules/110-TwoMeans/index.qmd), and [those between more than two means(ANOVA)](../../../20-Inference/Modules/130-ThreeMeansOrMore/index.qmd). 
...
![AI Detectors](../../../../../materials/images/ai-bias.png){#fig-ai-detectors-are-biased width="620px" height="400px"}
:::
...
cite_packages(
  ...
  out.dir = ".",
  out.format = "html",
  pkgs = c("TeachHist",  "TeachingDemos", "visualize", "naniar", "visdat", "tinytable", "sn")
) %>%
