...
#     wrap: 72
webr:
  packages: ['janitor','readr', 'tidyr', 'dplyr','mosaic', 'skimr','naniar', 'ggformula','ggridges', 'palmerpenguins', 'tidyplots', 'systemfonts', 'showtext']
format: 
  html: default
  revealjs: 
    theme: default
...
### {{< iconify icons8 idea >}} Inspiration: Column Chart

![Capital Cities](capitals-vs-rest.jpeg){#fig-column-chart-inspiration height=360 width=560}
...
### {{< iconify hugeicons api >}} ggformula and mosaic API
...
cool benefits such as "autocompletion" of variable names.
- The " *** " indicates what kind of graph you desire: histogram,
bar, scatter, density; 
- The "-------" is the name of your dataset that you want to plot with.
:::

### {{< iconify hugeicons api >}} ggplot API
...
- " ---- " is meant to imply text you supply. e.g. function names, data
...
- Note the change from `%>%` to `+` when adding a geom. Sigh. 
...
- Each Qual variable potentially has many levels as we saw in the [Nature of Data](../05-NatureData/index.qmd#what-are-data-types). 
...
### {{< iconify la r-project >}} Read Data
...
taxi_modified <- taxi_modified %>%
  dplyr::mutate(
    ...
    ## Variable "tip"
    tip = base::factor(tip, levels = c("yes", "no"), 
                 labels = c("yes", "no"),
                 ordered = TRUE),
    ...
    ## Variable "hour"
    hour = base::factor(hour, levels = c(0:23), labels = c(0:23), 
                  ordered = TRUE)) %>% 
...
- It is a good practice in Exploratory Data Analysis [to surmise the Experiment that lead to the gathering of this dataset]{style="background-color:yellow;"}
...
### {{< iconify noto-v1 spider-web >}} Playground-1
...
- (Future) The counts of `tip` are very imbalanced and if we are to setup a model for that ([logistic regression](../../../30-Modelling/Modules/30-LogReg/index.qmd)) we would need to  very carefully subset the data for `training` and `testing` our model. 
...
## Showing "per capita" percentages
## Better labelling of Y-axis in percentages
taxi_modified %>% 
  gf_percents(~ local, fill = ~ tip, 
  position = "fill") %>% 
...
### {{< iconify noto-v1 spider-web >}} Playground-2
...
### {{< iconify noto-v1 spider-web >}} Playground-3
...
- `fill`: gives us a per-group proportion of another Qual variable for a chosen Qual variable. This chart view is useful in [Inference for Proportions](../../../20-Inference/Modules/190-TwoProp/index.qmd);
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula41
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula41
#| echo: false
```
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula42
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula42
#| echo: false
```
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula43
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula43
#| echo: false
```
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula44
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula44
#| echo: false
```
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula45
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula45
#| echo: false
```
...
::: {.column}
```{r}
#| label: bar-tip-others-ggformula46
#| eval: false
...
:::{.column}
```{r}
#| ref-label: bar-tip-others-ggformula46
#| echo: false
```
...
### {{< iconify noto-v1 spider-web >}} Playground-4
...
- This is usually mechanized by a Stat Test called a [Single Proportion Test](../../../20-Inference/Modules/180-OneProp/index.qmd) 
- Or, when we have more than one, a [Multiple Proportion Test](../../../20-Inference/Modules/190-TwoProp/index.qmd). 


## {{< iconify openmoji person >}} Your Turn

1.  Click on the Dataset Icon, and unzip that archive. Try to make Bar plots with each of them, using one or more Qual variables. <a href="../../../../../materials/Data/qdd-data.zip"><i class="fa-solid fa-database"></i> Datasets</a>
...
1. Songs by Kishore Kumar: <https://sunilslists.com/hindi-songs/luminaries-hindi-songs/kishore-kumar-songs-all>
...
![Zipf's Law](../../../../../materials/images/zipf.png){#fig-zipfs-law height=360 width=540}


#### Zipf's Law

- Since the X-axis in bar charts is *Qualitative* (the bars don't touch, remember!) it is possible to sort the bars at will, based on the levels within the Qualitative variables. 
See the approx [Zipf's Law distribution](https://www.wikiwand.com/en/Zipf%27s_law) for the English alphabet above
- In @fig-zipfs-law, the letters of the alphabet are "levels" within a Qualitative variable
- these levels have been sorted based on the frequency or count! 
- This is what [Sherlock Holmes might have done](https://sherlock-holm.es/stories/pdf/a4/1-sided/danc.pdf), 
- Or the method how they cracked the code to the treasure in [this story.](https://www.youtube.com/watch?v=hsKVFeywcRY)
