...
Small dataset of 17 rows. [Only SMI peasants will interview 17 elders from their own family and call that `primary research`.]{.black .bg-pink}
...
```{r}
#| label: data-modified-and-saved-1
#| echo: true
...
# Two "NA" entries exist
 ...
 # arrange the Qual variables first, Quant next
  dplyr::relocate(where(is.factor)) # default relocation is at left

opening_modified
glimpse(opening_modified)   

```
The levels `[0,1]`, while good for computers, don't quite give a human feel to the data. Let us `recode` the binary factors above into `Yes/No` `Female/Male`, or `Pain/NoPain` etc:

```{r}
#| label: data-modified-and-saved-2
#| echo: true
...
# Two "NA" entries exist
  ...
  mutate(hand_pain = factor(hand_pain, levels = c(0,1),
                            labels = c("No Pain","Yes Pain")),
         hand_illness = factor(hand_illness, levels = c(0,1),
                               labels = c("No Illness","Yes Illness")),
         group = as_factor(group),
...
## First Plot modified
  ...
  gf_labs(
    title = "Opening Food Packages",
    subtitle = "Times take by Older People",
    x = "Time in seconds", 
    y = "Type of Product",
