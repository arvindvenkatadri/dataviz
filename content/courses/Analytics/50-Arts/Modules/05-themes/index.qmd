...
We may have been using `ggplot2` for a while now, but there is always more to learn about the aesthetics of our plots. In this module, we will explore how to use different fonts and themes to make our plots look more polished and professional. We will also learn how to use the `ggtext` package to add formatted text to our plots, and how to use the `ggforce` package to add additional functionality to `ggplot2`. We will explore how to use the `ggdist` package to add uncertainty visualizations to our plots, and how to use the `patchwork` package to combine multiple plots into a single output.
...
#| echo: fenced
#| 
font_add_google(family = "Gochi Hand", name = "gochi")
