...
My directory tree structure is:

- Project root
  - fonts
    - Alegreya-Regular.ttf ( for example)
...
Now, we can add these fonts to our R session as follows:
```{r}
...
Now that we have the fonts downloaded, we can use them in our plots. We will use the `theme_xxxx()` function to set a default theme for our plots, and then we will modify the theme elements to use our desired fonts.
...
# These may use package-specific commands, as with `flextable` and `tinytable`
