...
|  |  |  |
|------------------------|--------------------------|----------------------|
| <a href="./files/forward-selection-1.qmd"><i class="fa-brands        fa-r-project"></i> Multiple Regression - Forward Selection</a>   | <a href="./files/backward-selection-1.qmd"><i class="fa-brands fa-r-project"></i> Multiple Regression - Backward Selection</a>   | |
...
based on the mileage done, could we predict the resale price of a used
car? These are typical problems in Linear Regression.
...
In @eq-linear-model-1, $\beta_0$ is the *intercept* and $\beta_1$ is the
slope of the linear fit, that **predicts** the value of y based the
value of x. Each prediction leaves a small "residual" error between the
actual and predicted values. $\beta_0$ and $\beta_1$ are calculated
based on minimizing the *sum of square*s of these residuals, and hence
this method is called "ordinary least squares" (**OLS**) regression.

![Least Squares](../../../../../materials/images/OLS.png){#fig-least-squares height="280px"}

The net *area* of all the shaded squares is minimized in the calculation
of $\beta_0$ and $\beta_1$. As per Lindoloev, many statistical tests,
going from one-sample `t-tests` to `two-way ANOVA`, are special cases of
this system. Also see [Jeffrey Walker "A
...
::: callout-note
## Philosophical Word
Why would we want to ***add (weighted) variables together*** to predict a target variable? Isn't that a strange occupation? 
Think of how we may consult different people when we are embarked on making a decision. Do all opinions or inputs amount to the same? No; we attach more significance to the opinions of *people we trust*, and whose advice correlates with our (past) success, and less so to the opinions of other people. 
Hence a *weighted average* seems to be a very natural thing, a converting the [**Wisdom of the Crowds**](https://www.reddit.com/r/todayilearned/s/hQw5avD26b) to a *wisdom of the variables*. 
...
4.  We then find probability **p**($\beta_1 = Estimated\ Value$ **when
    the NULL Hypothesis** is **assumed** TRUE). This is the **p-value**.
    If that probability is **p\>=0.05**, we say we "cannot reject" $H_0$
    and there is unlikely to be significant linear relationship.
...
When does a Linear Model work? We can write the assumptions in Linear
Regression Models as an acronym, **LINE**:\
1. **L**: $\color{blue}{linear}$ relationship between variables 2.
**I**: Errors are **independent** (across observations)\
3. **N**: $y$ is $\color{red}{normally}$ distributed at each "level" of
$x$.\
4. **E**: $y$ has the same variance at all levels of $x$. No
*heteroscedasticity*.\

![OLS Assumptions](../../../../../materials/images/ols_assumptions.png){#fig-ols-assumptions height="280px"}
...
The target variable $y$ is modelled as a *normally* distribute variable **[whose mean depends upon a linear combination of predictor variables]{style="background-color:yellow;"}** $x$, and whose variance is $\sigma^2$.
:::
...
```{r}
#| label: fig-ggpairs-plots
#| fig-cap: "Pairs Plots with GGally"
#| fig-subcap:
#| - Quant Predictors Set 1
#| - Quant Predictors Set 2
#| - Quant Predictors Set 3
#| - Qual Predictors
#| code-fold: true
#| layout-ncol: 2
#| 
...
See @fig-ggpairs-plots-1. Clearly, `rm` (avg. number of rooms) is a big
determining feature for median price `cmedv`. This we infer based on the
large correlation of `rm` with`cmedv`, $0.696$. The variable`age`
(proportion of owner-occupied units built prior to 1940) may also be a
significant influence on `cmedv`, with a correlation of $-0.378$.

From @fig-ggpairs-plots-2, none of the Quant variables
`rad, zn, indus, tax` have a overly strong correlation with `cmedv`. .

From @fig-ggpairs-plots-3, the variable `lstat` (proportion of lower
classes in the neighbourhood) as expected, has a strong (negative)
correlation with `cmedv`; `rad`(index of accessibility to radial
highways), `nox`(nitrous oxide) and `crim`(crime rate) also have fairly
large correlations with `cmedv`, as seen from the pairs plots.

Among the *Qualitative predictor variables*, access to the Charles river
(`chas`) does seem to affect the prices somewhat, as seen from
@fig-ggpairs-plots-4. While not too many properties can be near the
Charles River (for obvious reasons) the box plots do seem to show some
dependency of `cmedv` on `chas`.
...
### {{< iconify flat-color-icons workflow >}} Correlations using Error Bar Plots

As we did with understanding [Change and
Correlations](../../../10-Descriptive/Modules/30-Change/#the-errorbar-plot-for-correlations),
we can quickly calculate all correlations of predictor variables with
the target variable `cmedv` and plot these in an error-bar plot. This
gives us a quick overview of which predictor variables have strong
correlations with the target variable.

```{r}
#| label: fig-error-bar-plot
#| fig-cap: "Correlation of Quantitative Predictors with Target Variable cmedv"
#| code-fold: true
...
```{r}
#| label: fig-cor-test-purrr
#| fig-cap: "Correlation Tests of All Quantitative Predictors vs Target Variable"
#| code-fold: true
#| 
ggplot2::theme_set(new = theme_custom())
...
:::::: {.panel-tabset .nav-pills style="background: whitesmoke;"}
#### {{< iconify mingcute code-fill >}} Model
...
-   The **effect size** of `rm` on predicting `cmedv` a (slope) value of
    ...
    one room increase in `rm`, we have a USD $90997$ increase in median
    price `cmedv`.
-   The **F-statistic** for the Linear Model is given by $F = 474.3$,
    which is very high. (We will use the F-statistic again when we do
    Multiple Regression.)
-   The `R-squared` value is $R^2 = 0.48$ which means that `rm` is able
    to explain about half of the trend in `cmedv`; there is substantial
    variation in `cmedv` that is still left to explain, an indication
    that we should perhaps use a richer model, with more predictors.
    These aspects are explored in the Tutorials.
:::

#### {{< iconify carbon forecast-hail >}} Forecasting
...
```{r}
#| label: tbl-housing-lm-predict
#| tbl-cap: "Predictions of cmedv"
#| tbl-subcap: 
#| - "For new values of rm"
#| - "For existing values of rm"
#| layout-ncol: 2
#| code-fold: true
#| 
...
```{r}
#| label: fig-housing-lm-plot
#| fig-cap: "Linear Model: Price vs Average no. of Rooms"
#| code-fold: true
#| warning: false
#| 
ggplot2::theme_set(new = theme_custom())
...
housing_lm_augment %>% 
  ...
  gf_point(.fitted ~ rm, shape = ~ "fitted", size = 4,
            color = "grey30") %>%
  gf_annotate(geom = "segment",
  ...
  gf_refine(
    scale_x_continuous(limits = c(2.5, 10),
                       expand = c(0, 0)),
    scale_shape_manual(values = c(fitted = 9), 
...
For any new value of `rm`, we go up to the [vertical blue line]{.black .bg-blue} and read off the predicted median price by following the [horizontal blue line]{.black .bg-blue}. That is how the model is used (by hand).

#### {{< iconify mdi thinking >}} Intuitive {#sec-lm-intuitive}
...
```{r}
#| label: fig-housing-lm-intuitive1
#| fig-cap: "Linear Model: Price vs Average no. of Rooms: Various Candidate Regression Lines"
#| fig-subcap:
#| - Slope = 5, Intercept = -30
#| - Slope = 2, Intercept = -15
#| - Slope = 0, Intercept = 0
#| - Slope = -2, Intercept = 10
#| - Slope = -5, Intercept = 30
#| - Slope = 0, Intercept = mean(cmedv)
#| 
#| code-fold: true
#| layout-ncol: 3
#| 
...
```{r}
#| label: fig-housing-lm-intuitive2
#| fig-cap: "Linear Model: Price vs Average no. of Rooms"
#| fig-subcap:
#| - "NULL Hypothesis: No relationship between Price and Rooms (SST)"
#| - "Alternate Hypothesis: Linear relationship between Price and Rooms (SSE)"
#| - "Residual Error after Modelling (SSR)"
#| code-fold: true
#| layout-ncol: 3
#| warning: false
#| 
...
In @fig-housing-lm-intuitive2-1 the *horizontal* [blue line]{.bg-blue
.black} is the overall mean of `cmedv`, denoted as $\mu_{tot}$. The
vertical [green lines]{.bg-green .black} to the points show the
departures of each point from this overall mean, called
[**residuals**]{style="background-color: yellow;"}. The sum of *squares* of these residuals in @fig-housing-lm-intuitive2-1 is called the [**Total Sum of Squares** (SST)]{style="background-color: yellow;"}.

$$
SST = \Sigma (y - \mu_{tot})^2
$$ {#eq-SST}

In @fig-housing-lm-intuitive2-2, the vertical [red lines]{.bg-red
.black} are the residuals of each point from the potential line of fit. The sum of the *squares* of these lines is called the [**Total Error Sum of Squares** (SSE)]{style="background-color: yellow;"}.

$$
SSE = \Sigma [(y - a - b * rm)^2]
$$ {#eq-SSE}

And in @fig-housing-lm-intuitive2-3, the vertical [grey lines]{.bg-gray .black} are the residuals (of each point) from the model line to the overall mean. The sum of the *squares* of these lines is called the [**Regression Sum of Squares** (SSR)]{style="background-color: yellow;"}.
$$
SSR = \Sigma [(\hat{y} - \mu_{tot})^2]
$$ {#eq-SSR}

It is straightforward to derive and show that:
$$
SST = SSR + SSE
$$ {#eq-SST-SSR-SSE}
...
```{r}
#| label: fig-housing-lm-intuitive3
#| fig-cap: "Linear Model: Price vs Average no. of Rooms: Error vs Slope"
#| code-fold: true
#| warning: false
#| 
ggplot2::theme_set(new = theme_custom())
...
To be written up. 
We can use the F-statistic to test the hypothesis that there is a linear relationship between `cmedv` and `rm`. The Null Hypothesis is that there is no relationship, and the Alternate Hypothesis is that there is a linear relationship. 

#### {{< iconify material-symbols slideshow-sharp >}} Linear Models by Hand
...
which is computed using the formula for the *f-distribution* in R.

::: callout-note
### Distribution of F

Why does the statistic $F$ have an F-distribution? This is because the
F-distribution is the distribution of the ratio of two scaled
Chi-squared distributions. In our case, both $SSR/df_{SSR}$ and
$SSE/df_{SSE}$ are scaled Chi-squared distributions, as they are sums of
squares of normally distributed variables (the residuals). The ratio of
these two independent Chi-squared variables, each divided by their
respective degrees of freedom, follows an F-distribution with degrees of
freedom $df_1 = df_{SSR}$ and $df_2 = df_{SSE}$. This is a fundamental
result in statistics that underpins the use of the F-test in regression
analysis.
:::

As with our hypothesis tests, we set the significance level to 0.95, and
quote the two relevant degrees of freedom as parameters to `qf()` which
...
#| fig-cap: "F Distribution with Critical Value"
#| code-fold: true
#| 
...
mosaic::xpf(q = F_crit, 
  ...
  gf_annotate(geom = "label", x = 3, y = 1.6, size = 4,
            label = "F Critical Value = 3.86", colour = "purple") %>% gf_annotate("curve", 
            x = 3, y = 1.5, 
            xend = F_crit - 0.025, yend = 1,curvature = 0.5,
            color = "purple", linewidth = 0.5, 
            arrow = arrow(length = unit(0.25,"cm"))) %>% 
  gf_labs(title = "F Distribution",
          subtitle = paste("df1 =", df_SSR, ", df2 =", df_SSE),
          x = "F value", y = "Density") %>% 
  gf_refine(scale_x_continuous(limits = c(0, 5), expand = c(0,0)),
...
The point-range plot helps to avoid what has been called
["within-the-bar
bias"](https://graphics.cs.wisc.edu/Papers/2014/CG14/Preprint.pdf). The
estimate is just a value, which we might plot as a *bar* or as a
*point*, with uncertainty error-bars.

Values **within the bar** are not more likely!! This is the bias that
the point-range plot avoids.
...
```{r}
#| label: fig-looking-at-residuals
#| fig-cap: "Looking at Residuals"
#| fig-subcap:
...
The residuals @fig-looking-at-residuals-1 are not quite "like the night sky", i.e. not random enough; there seems to be some pattern still in them. These point to the need for a richer model, with more
predictors. 

The "trend line" of residuals vs predictors @fig-looking-at-residuals-1 shows a U-shaped pattern, indicating significant nonlinearity: there is a curved relationship in the graph. The solution can be a **nonlinear
...
The Q-Q plot of residuals @fig-looking-at-residuals-4 has significant deviations from the normal quartiles. 
...
There is a very neat package called `ggstatsplot`[^3] that allows us to
...
package [^4], which is based on the methods discussed in [Judd et al](https://www.dataanalysisbook.com/index.html).
...
in [Judd et al.](https://www.dataanalysisbook.com/index.html)

#### {{< iconify ic twotone-rule >}} Workflow: Model Diagnostic Plots
...
```{r}
#| label: fig-using-base-r
#| fig-cap: "Using Base R for Model Diagnostics"
#| fig-subcap:
#| - "Residuals vs Fitted Values"
#| - "Normal Q-Q Plot"
#| - "Scale-Location Plot"
...
packages](https://exts.ggplot2.tidyverse.org/gallery/) named `{{lindia}}` also has a crisp command to plot these diagnostic graphs.

```{r}
#| label: fig-using-lindia-package
#| fig-cap: "Using the lindia package for Model Diagnostics"
#| message: false
...
y and $x_i$. Note that this is a vector dot-product, or inner-product,
taken with a *vector* of input variables $x_i$ and a vector of weights,
$\beta_i$. Together, the RHS of that equation defines an n-dimensional
...
There are three ways[^5] to include more predictors:

-   **Backward Selection**: We would typically start with a **maximal
    model**[^6] and progressively simplify the model by knocking off
...
The first two are covered in the @sec-linreg; Mixed Selection
we will leave for a more advanced course. But for now we will first use
just one predictor `rm`(Avg. no. of Rooms) to model housing prices.
...
[^3]: <https://indrajeetpatil.github.io/ggstatsplot/reference/ggcoefstats.html>

[^4]: <https://github.com/UCLATALL/supernova>
...
[^6]: Michael Crawley, *The R Book, Third Edition 2023. Chapter 9.
    Statistical Modelling*
