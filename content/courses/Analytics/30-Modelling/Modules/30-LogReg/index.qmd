...
We saw with the [**General Linear Model**](../20-LinReg/index.qmd#assumptions-in-linear-models) that it models the **mean** of
...
###### 2. Predictors and Weights: 

We have quantitative **predictors** so
...
###### 3. Making the LHS continuous? 

What can we try? In dummy encoding our target variable, we found a range of \[0,1\], [which is the same as the range for a **probability** value]{.black .bg-yellow}! Could we try to use ***probability of the outcome*** as our target, even though we are interested in binary outcomes themselves? This would still leave us with a range of $[0,1]$ for the target variable, as before.
...
#| fig-cap: "Variance vs Mean for Binomial Distribution"
#| echo: false
ggplot2::theme_set(new = theme_custom())
...
regression](../20-LinReg/index.qmd#sec-assumptions-in-linear-models).
...
**Error/Residual Distributions with Odds targets**. See the plot below. Odds are
...
###### 6. Putting it all together: Logistic Regression

So finally in our model, instead of modeling *levels/labels of the Qual target variable events (i.e Yes / No )*, we will use $log(odds)$ of the events themselves, also known as the **logit**, defined as:
...
mutate(default = if_else(default == "No", -1000, 1000)) %>%
  ...
  gf_annotate(geom = "text", y = -250, x = 2000, 
            label = "Logistic Regression\n model") %>%
  ##
  ## See <https://stackoverflow.com/questions/12514612/how-to-annotate-ggplot-with-latex#comment69094909_40528496>
  ##annotate(geom='text', x=3, y=3, label=Tex("$\\hat{Y} = B_0 + B_1X_1", output='character'), parse=TRUE)
  ##
  gf_annotate(geom = "label", x = 2000, y = -25,
    label = TeX("$y = beta_0 + beta_1 * balance", output='character'),
    parse = TRUE, color = "black", fill = "moccasin"
  ) %>% 
  gf_refine(scale_color_fermenter(palette = "RdBu",
                                  guide = "none")) %>% 
  gf_annotate("text", x = 300, y = 900, label = "P(default) = 1") %>% 
  gf_annotate("text", x = 300, y = -900, label = "P(default) = 0")
...
parameters as `predictors`, suitably (re)naming them too. We will also
...
```{r}
#| label: fig-pairs-plot-1
#| fig-cap: "Pairs Plot for Cancer Data"
#| code-fold: true
#| message: false
ggplot2::theme_set(new = theme_custom())
...
cancer_modified_sample %>% 
  gf_smooth(diagnosis_malignant ~ radius_mean, 
    method = glm,
    method.args = list(family = "binomial"),
    se = FALSE,
    color = "black", linewidth = 1.5
  ) %>%
  gf_refine(scale_colour_brewer(palette = "Set1", direction = -1)) %>%
...
```{r}
#| label: fig-point-projection
#| fig-cap: "Point Projection and Probability Lookup"
#| fig-subcap:
#| - "Point Projection Model #1"
#| - "Probability Lookup Model #1"
#| - "Point Projection Model #2"
#| - "Probability Lookup Model #2"
#| code-fold: true
#| layout-ncol: 2
#| 
ggplot2::theme_set(new = theme_custom())
...
plot_dat %>% 
  ...
  gf_refine(scale_color_brewer(palette = "Set1", direction = -1)) %>%
## Overlaid regression curves
...
- The internals of Logistic Regression seem quite different compared to those for Linear Regression. ***However***, when we use the [LINE assumptions]() to model the errors as *Gaussian* in [Linear Regression](), and follow through with a [*probabilistic analysis*](), we can conclude that the *Ordinary Least Squares* method we saw there is another manifestation of MLE! Here is a tutorial on [MLE](files/index.qmd) for the intrepid!
...
1. Paul Roback & Julie Legler. (January 26, 2021). *Beyond Multiple Linear Regression: Applied Generalized Linear Models and Multilevel Models in R* <https://bookdown.org/roback/bookdown-BeyondMLR/>
