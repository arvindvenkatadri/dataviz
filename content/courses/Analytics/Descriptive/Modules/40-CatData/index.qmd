...
packages for our purposes. Then we will see how they can be visualized. In most cases, we will also see how we can create the Contingency Table as a `tibble` or `data frame` for further analysis.
...
::: callout-important
### Workflow
So...in the end, it does seem that we could use mostly `vcd::structable()` or `ftable()` to create Contingency Tables, and then convert them to `tibble` form if needed.
